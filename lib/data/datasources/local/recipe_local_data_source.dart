...
class RecipeLocalDataSourceImpl implements RecipeLocalDataSource {
  ...
  Future<RecipeVersionModel?> getRecipeVersion(String id) async {
    final db = await _dbHelper.database;
    final maps = await db.query('recipe_versions', where: 'id = ? AND isDeleted = ?', whereArgs: [id, 0]);
  ...
  Future<List<IngredientModel>> _getIngredientsForVersion(Database db, String versionId) async {
    final maps = await db.query('ingredients', where: 'recipeVersionId = ? AND isDeleted = ?', whereArgs: [versionId, 0]);
    return maps.map((map) => IngredientModelExtension.fromMap(map)).toList();
  }

  Future<List<StepModel>> _getStepsForVersion(Database db, String versionId) async {
    final maps = await db.query('steps', where: 'recipeVersionId = ? AND isDeleted = ?', whereArgs: [versionId, 0]);
    return maps.map((map) => StepModelExtension.fromMap(map)).toList();
  }

  @override
  Future<void> saveRecipe(RecipeModel recipe, RecipeVersionModel version) async {
    final db = await _dbHelper.database;
    await db.transaction((txn) async {
      await txn.insert('recipes', recipe.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
      
      await txn.insert('recipe_versions', version.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);

      for (final ingredient in version.ingredients) {
        await txn.insert('ingredients', ingredient.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
      }
      for (final step in version.steps) {
        await txn.insert('steps', step.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
      }
    });
  }
  ...
  Future<void> deleteRecipe(String id) async {
    ...
    await db.update('recipes', {'isDeleted': 1, 'updatedAt': now}, where: 'id = ?', whereArgs: [id]);
    await db.update('recipe_versions', {'isDeleted': 1}, where: 'recipeId = ?', whereArgs: [id]);
    // Also mark ingredients and steps as deleted
    final versions = await db.query('recipe_versions', where: 'recipeId = ?', whereArgs: [id], columns: ['id']);
    for (final version in versions) {
      final versionId = version['id'] as String;
      await db.update('ingredients', {'isDeleted': 1, 'updatedAt': now}, where: 'recipeVersionId = ?', whereArgs: [versionId]);
      await db.update('steps', {'isDeleted': 1, 'updatedAt': now}, where: 'recipeVersionId = ?', whereArgs: [versionId]);
    }
  }
}
