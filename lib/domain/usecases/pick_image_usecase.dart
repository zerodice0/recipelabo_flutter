...
import 'package:saucerer_flutter/core/services/image_storage_service.dart';
...
class PickImageUseCase {
  ...
  Future<String?> call(ImageSourceType sourceType, {String? customFileName}) async {
    try {
  ...
  Future<List<String>> pickMultipleImages({int maxImages = 5}) async {
    try {
      ...
      for (final XFile pickedFile in limitedFiles) {
        final File imageFile = File(pickedFile.path);
        final String savedPath = await _imageStorageService.saveImageFile(imageFile);
