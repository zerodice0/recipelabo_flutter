...
class TranslationUpdater {
  ...
  static const String sheetRange = 'Sheet1!A2:E';
  ...
  static Future<TranslationUpdater> create({
    ...
    if (serviceAccountJson != null) {
      ...
      throw Exception(
        ...
        'GOOGLE_SERVICE_ACCOUNT_JSON 환경변수를 사용하여 서비스 계정으로 인증하세요.',
      );
    } else {
      throw Exception(
        'Google Sheets API 인증이 필요합니다. '
        'GOOGLE_SHEETS_API_KEY 또는 GOOGLE_SERVICE_ACCOUNT_JSON 환경변수를 설정하세요.',
      );
    }
  ...
  Future<void> exportToCsv(List<TranslationRow> translations) async {
    final csvData = [
      ...
      ...translations.map((t) => [t.key, t.korean, t.english, t.japanese, t.description]),
    ];
...
환경변수:
  GOOGLE_SHEETS_API_KEY          Google Sheets API 키 (필수)
  GOOGLE_SERVICE_ACCOUNT_JSON    서비스 계정 JSON (선택적, API 키 대신 사용)
