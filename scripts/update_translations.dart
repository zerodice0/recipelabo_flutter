...
class TranslationUpdater {
  static const String defaultSheetId = '1q3T5hPEshaAifT5K9g0L-2yqPH4zv62x-43Z1u-cZns';
  static String get apiKey => Platform.environment['GOOGLE_SHEETS_API_KEY'] ?? '';
  ...
  Future<String> downloadSheetAsCsv() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to download sheet: HTTP ${response.statusCode}');
      }
    } finally {
      client.close();
    }
  ...
  Map<String, Map<String, String>> parseCsvToTranslations(String csvData) {
    ...
    final koIndex = headers.indexWhere((h) => h.contains('Korean') || h.contains('ko'));
    final enIndex = headers.indexWhere((h) => h.contains('English') || h.contains('en'));
    final jaIndex = headers.indexWhere((h) => h.contains('Japanese') || h.contains('ja'));
    final statusIndex = headers.indexOf('Status');
    
    if (keyIndex == -1 || koIndex == -1 || enIndex == -1 || jaIndex == -1) {
      throw Exception('Required columns not found. Expected: Key, Korean, English, Japanese');
    }
    
    print('📍 Column mapping: Key=$keyIndex, Ko=$koIndex, En=$enIndex, Ja=$jaIndex');
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      final ko = row.length > koIndex ? row[koIndex]?.toString().trim() ?? '' : '';
      final en = row.length > enIndex ? row[enIndex]?.toString().trim() ?? '' : '';
      final ja = row.length > jaIndex ? row[jaIndex]?.toString().trim() ?? '' : '';
  ...
  Future<void> generateArbFiles(Map<String, Map<String, String>> translations) async {
  ...
  Future<void> generateArbFile(String locale, Map<String, Map<String, String>> translations) async {
    ...
    for (final entry in translations.entries) {
      final key = entry.key;
      final translations_map = entry.value;
      final value = translations_map[locale];
      ...
      if (newSection != currentSection) {
        ...
        arbData['@@${newSection}_section'] = _getSectionTitle(newSection, locale);
      }
    ...
    if (dryRun) {
      ...
      print(jsonContent.substring(0, jsonContent.length > 300 ? 300 : jsonContent.length));
      print('...');
    } else {
  ...
  String _getSectionTitle(String section, String locale) {
    final titles = {
      ...
      'empty': {'ko': '빈 상태 메시지', 'en': 'Empty State Messages', 'ja': '空の状態メッセージ'},
      ...
      'seasoning': {'ko': '조미료 관리', 'en': 'Seasoning Management', 'ja': '調味料管理'},
...
Future<void> main(List<String> arguments) async {
  ...
  if (sheetId == 'YOUR_SHEET_ID_HERE') {
    ...
    print('💡 Please provide sheet ID as argument or update defaultSheetId in script');
    printUsage();
    exit(1);
  }
