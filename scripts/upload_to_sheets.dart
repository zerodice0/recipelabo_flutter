...
class SheetsUploader {
  ...
  static Future<SheetsUploader> create() async {
    String? serviceAccountPath = Platform.environment['GOOGLE_SERVICE_ACCOUNT_JSON'];
    
    // 환경변수가 없으면 기본 경로 사용
    serviceAccountPath ??= 'scripts/service-account-key.json';
    
    final serviceAccountFile = File(serviceAccountPath);
    if (!serviceAccountFile.existsSync()) {
      throw Exception('''
서비스 계정 JSON 파일을 찾을 수 없습니다: $serviceAccountPath

다음 중 하나를 수행해주세요:
1. scripts/service-account-key.json 파일 생성
2. 환경변수 설정: GOOGLE_SERVICE_ACCOUNT_JSON=path/to/key.json
...
''');
  ...
  Map<String, String> loadHardcodedStrings() {
    ...
    return {
      ...
      'turnOffInstructions': '1. 아이폰 설정 앱 열기\\n2. 알림 > Saucerer 선택\\n3. 알림 허용 끄기',
  ...
  String generateEnglishTranslation(String key, String korean) {
    final translations = {
      ...
      'backgroundAppRefreshDescription': 'Set up for accurate timer notifications',
    };
