...
class SheetsApiKeySync {
  ...
  static const String sheetRange = 'multi_language!A2:E'; // A2부터 E열까지 (헤더 제외)
  ...
  Future<List<List<dynamic>>> fetchTranslationData() async {
    ...
    if (response.statusCode != 200) {
      throw Exception(
          'Google Sheets API 호출 실패: ${response.statusCode}\\n${response.body}');
    }
    ...
    if (values == null || values.isEmpty) {
      throw Exception(
          'Google Sheets에서 데이터를 찾을 수 없습니다. 스프레드시트에 데이터가 있는지 확인해주세요.');
    }
  ...
  Future<void> generateArbFiles(List<List<dynamic>> translationData) async {
    ...
    for (final locale in ['ko', 'en', 'ja']) {
      ...
      final jsonString =
          const JsonEncoder.withIndent('  ').convert(arbData[locale]);
...
Future<void> main() async {
  ...
  if (apiKey == null || apiKey.isEmpty) {
    ...
    print('사용법:');
    print(
        'GOOGLE_SHEETS_API_KEY=your_api_key dart scripts/sync_from_sheets_api_key.dart');
    print('');
